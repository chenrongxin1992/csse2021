...
layui.define(["element","jquery"],function(exports){
	...
	Tab.prototype.render = function() {
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="/manage/main"><i class="layui-icon" data-icon=""></i><cite>后台首页</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height()-210);
	...
	Tab.prototype.tabAdd = function(_this){
		...
		var openTabNum = that.tabConfig.openTabNum;
			...
			console.log('_this.attr(target)',_this.attr('target'))
			console.log('_this.attr(data-url)',_this.attr('data_url'))
		if(_this.attr("target") == "_blank"){
			...
			if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
				...
				console.log("_this.parent('.layui-nav-item').index()",_this.parent('.layui-nav-item').index())
				window.sessionStorage.setItem("curmenu", JSON.stringify(curmenu));  //当前的窗口
				console.log('tabFilter',tabFilter,that.getLayId(_this.find("cite").text()))
				element.tabChange(tabFilter, that.getLayId(_this.find("cite").text()));
				that.tabMove(); //顶部窗口是否可滚动
			}
		}
